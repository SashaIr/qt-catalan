...
@[simp]
theorem q_analog_eq_geom_sum {R : Type*} [DivisionRing R] (q : R) (n : ℕ) :
    q_analog n q * (1 - q) = (1 - q ^ n) := by
  induction n with
  | zero => simp [q_analog]
  | succ n ih =>
    rw [q_analog, add_mul, ih]; sorry
...
def q_binomial {R : Type*} [DivisionRing R] (n k : ℕ) (q : R) : R :=
  ...
  | k + 1 => match n with
            | 0     => 0
            | n + 1 => q_binomial n k q + q ^ (k + 1) * q_binomial n (k + 1) q

@[simp]
theorem q_binomial_zero_right {R : Type*} [DivisionRing R] (n : ℕ) (q : R) : q_binomial n 0 q = 1 := by
  simp [q_binomial]

@[simp]
theorem q_binomial_zero_left {R : Type*} [DivisionRing R] (k : ℕ) (q : R) : q_binomial 0 k q = (if k = 0 then 1 else 0) := by
  cases k <;> simp [q_binomial]

/-
The q-Pascal identity states that [n+1 choose k+1]_q = [n choose k]_q + q^(k+1) * [n choose k+1]_q.
-/
theorem q_pascal {R : Type*} [DivisionRing R] (n k : ℕ) (q : R):
...
theorem sum_inv_eq_q_binomial {R : Type*} [Field R] (n k : ℕ) (q : R) (hq : q ≠ 0) :
